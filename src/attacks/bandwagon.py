...
class BandwagonAttack:
    def __init__(self, target_iids: List[int], fake_users: Union[float, int], num_selected: int, seed: int = 42):
        """
        Initialize the Bandwagon Attack.
        -------
    ...
    def attack(self, im_real: csr_matrix) -> csr_matrix:
        ...
        selected_iids, _ = find_favorite_items(im_real, num_selected=self.num_selected)
        max_rating = im_real.data.max()
        im_fake[:, selected_iids] = max_rating
